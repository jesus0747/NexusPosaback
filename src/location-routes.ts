...
router.post("/nexus/accounts/:accountId/locations", requireAccountAccess(), async (req, res) => {
  const {
    name, address, state, timezone = "America/New_York",
  ...
  try {
    ...
    await db.insert(nexusTenants).values({
      ...
      state: state ?? null, timezone, currency, tax_rate: tax_rate as unknown as number,
...
router.put("/nexus/locations/:locationId", async (req, res) => {
  const { name, address, state, timezone, currency, tax_rate, tax_config, payment_config_id, status } =
    req.body as Record<string, string>;
  try {
...
const DEVICE_TYPES = ["POS", "KDS", "BAR", "HOST", "ADMIN_DISPLAY"] as const;

/** POST /nexus/locations/:locationId/devices — register a new device */
router.post("/nexus/locations/:locationId/devices", async (req, res) => {
  const { name, type = "POS" } = req.body as { name?: string; type?: string };
  if (!name?.trim()) return res.status(400).json({ error: "name is required" });
  if (!DEVICE_TYPES.includes(type as (typeof DEVICE_TYPES)[number]))
    return res.status(400).json({ error: `type must be one of: ${DEVICE_TYPES.join(", ")}` });

  const locationId = req.params.locationId;
  try {
    const [location] = await db.select({ account_id: nexusTenants.account_id })
      .from(nexusTenants).where(eq(nexusTenants.tenant_id, locationId));
    if (!location) return res.status(404).json({ error: "Location not found" });

    const device_id = newId("dev");
    const token = randomBytes(32).toString("hex");
    const [device] = await db.insert(nexusDevices).values({
      device_id,
      tenant_id:   locationId,
      location_id: locationId,
      account_id:  location.account_id ?? undefined,
      name:        name.trim(),
      type,
      token,
    }).returning();
...
/** PUT /nexus/devices/:deviceId — rename or toggle device */
router.put("/nexus/devices/:deviceId", async (req, res) => {
  ...
  try {
    ...
    await db.update(nexusDevices)
      .set(updates as Parameters<ReturnType<typeof db.update>["set"]>[0])
      .where(eq(nexusDevices.device_id, req.params.deviceId));
    const [device] = await db.select().from(nexusDevices)
      .where(eq(nexusDevices.device_id, req.params.deviceId));
    res.json(device ?? { ok: true });
  } catch (err) {
    res.status(500).json({ error: err instanceof Error ? err.message : "Failed" });
  }
});

/** DELETE /nexus/devices/:deviceId — permanently remove a device */
router.delete("/nexus/devices/:deviceId", async (req, res) => {
  try {
    await db.delete(nexusDevices).where(eq(nexusDevices.device_id, req.params.deviceId));
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: err instanceof Error ? err.message : "Failed" });
  }
...
/** PUT /nexus/users/:userId/change-pin — employee changes their own PIN */
router.put("/nexus/users/:userId/change-pin", async (req, res) => {
  const { tenant_id, current_pin, new_pin } = req.body as { tenant_id: string; current_pin: string; new_pin: string };
  if (!tenant_id || !current_pin || !new_pin) {
    return res.status(400).json({ error: "tenant_id, current_pin, and new_pin are required" });
  }
  if (!/^\d{4}$/.test(new_pin)) {
    return res.status(400).json({ error: "New PIN must be exactly 4 digits" });
  }
  try {
    const [user] = await db.select().from(nexusUsers).where(
      and(eq(nexusUsers.user_id, req.params.userId), eq(nexusUsers.tenant_id, tenant_id))
    );
    if (!user) return res.status(404).json({ error: "User not found" });
    if (!user.pin_hash) return res.status(400).json({ error: "No PIN set — use reset-pin first" });
    const expected = hashPin(req.params.userId, current_pin);
    if (expected !== user.pin_hash) return res.status(401).json({ error: "Current PIN is incorrect" });
    const new_hash = hashPin(req.params.userId, new_pin);
    await db.update(nexusUsers)
      .set({ pin_hash: new_hash, pin_reset_required: false, updated_at: new Date() } as Record<string, unknown>)
      .where(eq(nexusUsers.user_id, req.params.userId));
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: "Failed to change PIN", detail: String(e) });
  }
});
...
/** POST /nexus/auth/pin-only — identify employee by PIN alone (no user_id required) */
router.post("/nexus/auth/pin-only", async (req, res) => {
  const { tenant_id, pin } = req.body as { tenant_id: string; pin: string };
  if (!tenant_id || !pin) {
    return res.status(400).json({ error: "tenant_id and pin are required" });
  }
  if (!/^\d{4,6}$/.test(pin)) {
  ...
  try {
    ...
    // Find users that have access to this location
    const candidates = allUsers.filter((u) => {
      const perms = (u.location_permissions as Record<string, string>) ?? {};
      return !!perms[tenant_id] || u.role === "OWNER" || u.role === "MANAGER";
    });

    // Scan PIN hashes — O(n) where n = location staff count (always small)
    const matches: typeof candidates = [];
    for (const u of candidates) {
      if (!u.pin_hash) continue;
      const expected = hashPin(u.user_id, pin);
      if (expected === u.pin_hash) matches.push(u);
    }
    ...
    if (matches.length > 1) {
      // Multiple employees share the same PIN — admin must reassign unique PINs
      return res.status(409).json({ error: "PIN conflict — contact your manager to set a unique PIN" });
    }

    const user = matches[0];
    await db.update(nexusUsers)
      .set({ last_seen_at: new Date() })
      .where(eq(nexusUsers.user_id, user.user_id));

    const perms = (user.location_permissions as Record<string, string>) ?? {};
    const { pin_hash: _ph, ...safe } = user;
    res.json({
      ok: true,
      employee: { ...safe, location_role: perms[tenant_id] ?? user.role },
    });
  } catch (err) {
    res.status(500).json({ error: err instanceof Error ? err.message : "Failed" });
  }
...
router.get("/nexus/locations/:locationId/config", async (req, res) => {
  try {
    ...
    const { logo_url, phone, tax_id, receipt_header, receipt_footer, receipt_config, tip_config, tax_rate, state, ...locationInfo } = location;
    res.json({
      location: {
        ...locationInfo,
        state,
        business: { logo_url, phone, tax_id, receipt_header, receipt_footer, receipt_config, tip_config, tax_rate, state },
      },
      staff,
    });
  } catch (err) {
...
router.put("/nexus/locations/:locationId/config", async (req, res) => {
  const { logo_url, phone, tax_id, receipt_header, receipt_footer, receipt_config, tax_rate, tip_config, state } = req.body;
  try {
    const updates: Record<string, unknown> = { updated_at: new Date() };
